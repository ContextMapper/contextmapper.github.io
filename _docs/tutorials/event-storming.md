...
Context Mapper supports most of these concepts, and can therefore be used to document the output of an event storming workshop. Once captured in Context Mapper, 
the workshop results can be processed further, for instance to generate UML diagrams or service contracts.
...
The following examples illustrate how we modeled the domain events from our Event Storming:
...
The service operations above indicate that the result of a command is a certain event. For example: If the `CheckClaimDocumentation` command is performed, an event `ClaimRegistered`
will be the result.
...
The following example illustrates an Aggregate derived from the Event Storming above:
...
**Note:** Some Event Storming tutorials/guides also feature Entities instead of Aggregates. Typically this Entities become an Aggregate root (often the Aggregate even 
becomes the same name as the Aggregate root Entity). For the model in CML it does not matter if you work with Aggregates or Entities in your Event Storming. You have to create
an Aggregate in all cases. Within the Aggregate you can then create your Entity, as shown in the example above.

### Issues
At present, we do not have a language construct for issues since they mostly only used as notes for potential future model changes and are not further processed 
(do not influence the model structurally). However, you can capture them as comments, although it might make more sense to capture them in the issue tracking tool or 
Kanban board of the project straight away.

The Lakeside Mutual Event Storming above contains two issues for which we created the following comment in our CML model:
...
### User Roles
At present, there is no concept of a user role in the Context Mapper DSL; however, we used [Sculptor](http://sculptorgenerator.org/)'s `doc` comment which can be added to all 
domain objects. Thereby we declare the user roles on our commands:
...
### Views / Read Models
In the Event Storming above we did not work with read models and Context Mapper does not support read models explicitely yet. However, by just using separate Aggregates or Entities
it is of course possible that you define your read model. For example:
...
### Event Flow
In an Event Storming you order the events in the order they occur (in time). We did the same thing in the Event Storming above (Lakeside Mutual). The time line in the illustrated
graphic above proceeds from left to right. 

Context Mapper does currently not support to model this event flow (time) explicitely. For this reason we simply worked with comments that indicate which command is triggered next
after an event:
...
The order in which we listed the events/commands above and the comments `// triggers ...` model the time line in this example.
